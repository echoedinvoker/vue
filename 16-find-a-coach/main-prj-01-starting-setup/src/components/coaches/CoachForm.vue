  ...
  <form @submit.prevent="submitForm">
    ...
    <div class="form-control" :class="{ invalid: !rate.isValid }">
      ...
      <input
        ...
        @blur="validate('rate')"
      />
      <p v-if="!rate.isValid">Rate must over than zero.</p>
    </div>
    <div class="form-control" :class="{ invalid: !areas.isValid }">
      ...
      <p v-if="!areas.isValid">At least check one box.</p>
    </div>
...
export default {
  ...
  methods: {
    validate(input) {
      if (
        ['firstName', 'lastName', 'description'].find((el) => el === input) &&
        !this[input].val
      ) {
      ...
      if (input === 'rate' && (!this.rate.val || this.rate.val < 0)) {
        this[input].isValid = false;
        return;
      }
      if (input === 'areas' && this.areas.val.length === 0) {
        this[input].isValid = false;
        return;
      }
